...
updates_dir=/data/spiceos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have SpiceOS-VERSION-DATE-TYPE-* as format"
...
# Assume SpiceOS-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.spiceos.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.spiceos.updater/databases/updates.db" \
